...
prob = exp(prob)/(1+exp(prob))
#D = rbinom(n,size = 1 , prob)      #DR AIPW can be future development
D = (prob >0.5)*1
Y = D*2 + rowSums(X[,c(1:4)])*2 + rnorm(n)

Data_fun <- Data_GenX( X, 
                       alpha = c(0.2,0.2,0,0,0.3,0.3), beta = c(2,2,2,2,0,0), #alpha = treatment_scale, beta = outcome_scale
                       theta = 2, num_pi = 1, delta = 0,
                       linearY = FALSE, typeY = "pois")

Ori_Data=Data_fun$Data

X <- as.matrix(Ori_Data[,1:p])
D <- Ori_Data$D
Y <- Ori_Data$Y
